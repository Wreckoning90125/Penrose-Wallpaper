...
const PRESETS = [
  { name: 'BW', bg: [0, 0, 0], colors: () => Array.from({ length: MAX_COLORS }, (_, i) => [i & 1 ? 1 : 0, 0, 0]) },
  ...
  { name: 'Spectra', bg: [0.04, 0.005, 280], colors: k => pad(Array.from({ length: k }, (_, i) => [0.70, 0.16, (i * 360 / Math.max(k, 1) + 30) % 360])) },
...
export function buildPalette(presetIndex, colorCount, customColors = null) {
